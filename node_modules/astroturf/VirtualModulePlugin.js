...
class VirtualModulePlugin {
  ...
  static bootstrap(compilation, files) {
    ...
    if (compiler.resolverFactory.cache1) compiler.resolverFactory.cache1 = new WeakMap(); // >=4.36.1 this one is needed

    if (compiler.resolverFactory.cache2) compiler.resolverFactory.cache2.clear();
    return plugin;
  }
  ...
  augmentCompilerFileSystem(compiler) {
    ...
    if (!compiler.hooks) {
       ...
       */
      compiler.hooks.watchRun.tapAsync('astroturf', ({
        fileTimestamps
      }, callback) => {
