...
const read = (file, optsOrEncoding) => {
  const encoding = (optsOrEncoding == null ? void 0 : optsOrEncoding.encoding) || optsOrEncoding;
  file.atime = new Date();
...
class MemoryFs {
  constructor() {
    this.addFile = (p, data, updateMtime = false) => {
      p = path.normalize(p);
      const hash = md4(data);
      const existing = this.paths.get(p);
      const keepTime = !updateMtime && existing && existing.hash === hash;
      if (!keepTime) debug(`${existing ? 'modifying' : 'writing'} file ${path.relative(process.cwd(), p)} [${hash}]`);
    ...
    this.getPaths = () => this.paths;

    this.exists = (p, cb) => cb(this.existsSync(p));

    this.existsSync = p => this.paths.has(path.normalize(p));
    ...
    this.readFileSync = (p, optsOrEncoding) => {
      p = path.normalize(p);
      if (!this.existsSync(p)) throw new MemoryFileSystemError(errors.code.ENOENT, p, 'readFile');
    ...
    this.unlinkSync = p => this.paths.delete(p);
    ...
    this.paths = new Map();
    ['stat', 'readdir', 'rmdir', 'unlink', 'readFile', 'writeFile'].forEach(fn => {
      this[fn] = (...args) => {
        const cb = args.pop();
        let result;
  ...
  createReadStream(p, options) {
    ...
    options.end = options.end || data.length; // eslint-disable-next-line no-underscore-dangle
  ...
  createWriteStream(p) {
    ...
    const bl = [];
    let len = 0; // eslint-disable-next-line no-underscore-dangle
