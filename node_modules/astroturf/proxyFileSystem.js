...
const TO_PROXY = ['exists', 'readFile', 'writeFile', 'stat', 'unlink'].reduce(withSyncMethod, []).concat(['createReadStream', 'createWriteStream']);
/**
  ...
  TO_PROXY.forEach(method => {
    ...
    proxiedMethods[method] = function proxy(path, ...args) {
      ...
      if (!virtualFs[method]) {
        ...
        if (method.endsWith('Sync')) throw err;else return args.pop()(err);
      }
